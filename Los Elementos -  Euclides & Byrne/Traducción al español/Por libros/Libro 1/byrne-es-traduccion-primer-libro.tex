...
\small
\textit{Los Elementos} es una recopilación de trabajos previos de matemáticos como Pitágoras, Eudoxo y Teeteto. Euclides recopiló y organizó estos trabajos aproximadamente hace 2300 años en Alejandría.
\\[\baselineskip]
...
De la misma manera, el doble del cuadrado de, se expresa por $2 \cdot \drawUnitLine{AD}^2$.
...
\offsetPicture{2pt}{0pt}{\drawFromCurrentPicture{
	draw byNamedAngle(HGI);
	draw byNamedLineSeq(0)(GH,GI);
}}\\ \vskip 0.5\baselineskip entonces \\ \vskip 0.5\baselineskip
\offsetPicture{2pt}{0pt}{\drawFromCurrentPicture{
	draw byNamedAngle(BAC);
	draw byNamedLineSeq(0)(AB,AC);
}} $=$ 
...
\drawCurrentPictureInMargin
\problem[3]{E}{n}{cualquier triángulo \drawLine{DC,BC,BA,AD} si un lado \drawUnitLine{AD,DC}es mayor que otro \drawUnitLine{BC}, el ángulo opuesto al lado mayor es mayor que el ángulo opuesto al lado menor $\drawAngle{DBC,ABD} > \drawAngle{A}$.}
...
\drawCurrentPictureInMargin
\problem{S}{i}{en cualquier triángulo \drawLine[bottom]{CA,BC,AB} un ángulo \drawAngle{A} es mayor que otro \drawAngle{B} el lado \drawUnitLine{BC} opuesto al ángulo mayor es mayor que el lado \drawUnitLine{CA} opuesto al ángulo menor.}
...
\drawCurrentPictureInMargin
\problem[2]{S}{i}{desde cualquier punto (\drawPointL[middle][DE]{D}) dentro de un triángulo \drawLine[bottom]{CA,EC,BE,AB} se trazan lineas rectas a los extremos de uno de los lados (\drawSizedLine{AB}), la suma de esas líneas será menor que la de los otros dos lados del triángulo, y contendrán un ángulo mayor.}
...
cuya suma de dos cualesquiera sea mayor que la tercera, construir un triángulo cuyos lados sean respectivamente iguales a las lineas dadas.}
...
Traza \drawUnitLine{BC} en dos puntos cualesquiera de los lados del ángulo dado.
...
\drawCurrentPictureInMargin
\problem{S}{i}{un triángulo tiene dos lados respectivamente iguales a dos lados de otro (\drawUnitLine{AB} con \drawUnitLine{EF} y \drawUnitLine{AD} con \drawUnitLine{GE}), y si uno de los ángulos
...
el lado (\drawUnitLine{DB}) opuesto al mayor ángulo es mayor que el lado (\drawUnitLine{FG}) opuesto al ángulo menor.
}
