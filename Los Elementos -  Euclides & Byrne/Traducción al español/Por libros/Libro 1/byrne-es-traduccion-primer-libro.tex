...
Si los lados no son iguales, entonces uno de ellos \drawUnitLine{CA,CD} es mayor que el otro \drawUnitLine{BD}, y de él se corta $\drawUnitLine{CA} = \drawUnitLine{BD}$ \byref{prop:I.III}, y se dibuja \drawUnitLine{BC}.
...
\begin{center}
Tomar $\drawUnitLine{AB} = \drawUnitLine{CA}$ \byref{prop:I.III}
...
\begin{center}
Con el punto dado \drawPointL{A} como centro, en un lado de la línea, y a cualquier distancia \drawUnitLine{DB} capaz de extenderse al otro lado, traza con el compás \drawArc{O}. % improvement: the distance mentioned here seems not to be in the original drawing, it should either be drawn, or not be referenced graphically as DB
...
\drawCurrentPictureInMargin
\problem{S}{i}{el lado de un triángulo (\drawLine[bottom]{BE,EC,AC,AB}) es alargado, el ángulo externo (\drawFromCurrentPicture[middle][anglesECDpDCF]{
startAutoLabeling;
draw byNamedAngleSides(ECD,DCF)(CF);
stopAutoLabeling;
}) es mayor que cualquiera de los ángulos internos remotos (\drawAngle{B} or \drawAngle{A}).
}

\begin{center}
Toma $\drawUnitLine{BE} = \drawUnitLine{EC}$ \byref{prop:I.X};\\
dibuja \drawUnitLine{AE} y alarga hasta que $\drawUnitLine{ED} = \drawUnitLine{AE}$;\\
...
En la misma manera se puede demostrar que si \drawUnitLine{BC} se alarga, $\drawAngle{GCA} > \drawAngle{A}$ \\
...
\drawCurrentPictureInMargin
\problem[3]{E}{n}{cualquier triangulo \drawLine{DC,BC,BA,AD} si un lado \drawUnitLine{AD,DC}es mayor que otro \drawUnitLine{BC}, el angulo opuesto al lado mayor es mayor que el angulo opuesto al lado menor, es decir $\drawAngle{DBC,ABD} > \drawAngle{A}$.}
...
dibuja \drawSizedLine{CA} y \drawSizedLine{BC},\\
será \drawLine[bottom]{CA,BC,AB} el triángulo requerido.
...
\drawCurrentPictureInMargin
\problem{S}{i}{dos triangulos tienen dos lados del uno respectivamente iguales a dos lados del otro (\drawUnitLine{AB} con \drawUnitLine{EF} y \drawUnitLine{AD} con \drawUnitLine{GE}), y si uno de los ángulos
(\drawFromCurrentPicture[bottom]{
startAutoLabeling;
draw byNamedAngleSides(BAC,CAD)(AB,CA,AD);
stopAutoLabeling;
})
contenido por los lados iguales esmayor que el otro
(\drawFromCurrentPicture[bottom][angleFEG]{
startAutoLabeling;
draw byNamedAngleSides(FEG)(EF, GE);
stopAutoLabeling;
}),
el lado (\drawUnitLine{DB}) opuesto al mayor angulo es mayor que el lado (\drawUnitLine{FG}) opuesto al menor angulo.
}

\begin{center}
Crea $\drawFromCurrentPicture[bottom][angleBAC]{
startAutoLabeling;
...
\drawCurrentPictureInMargin
\problem{S}{i}{un triangulo tiene dos lados (\drawUnitLine[0.7cm]{AB} y \drawUnitLine[0.7cm]{CA}) respectivamente iguales a dos lados (\drawUnitLine{DE} y \drawUnitLine{FD}) de otro triangulo, pero con bases desiguales, el ángulo subtendido por la base mayor (\drawUnitLine{BC}) de uno, debe ser mayor que el ángulo subtendido por la base menor (\drawUnitLine{EF}) del otro.}

\begin{center}
$\drawAngle{A} =\mbox{, } > \mbox{ o } < \drawAngle{D}$
