...
\drawCurrentPictureInMargin
\problem{P}{aralelogramos}{ (\drawPolygon[bottom][polygonABDC]{ABIC, CDI}~and~\drawPolygon[bottom][polygonEFHG]{EFJ, JFHG}) en bases iguales, y entre las mismas paralelas, son iguales.}

\begin{center}
Dibuja \drawUnitLine{CE} y \drawUnitLine{DF},
 \\ $\drawUnitLine{CD} = \drawUnitLine{GH} = \drawUnitLine{EF}$, por \byref{prop:I.XXXIV,\hypref};
                     \\ $\therefore \drawUnitLine{CD} = \mbox{ y } \parallel \drawUnitLine{EF}$;
 \\ $\therefore \drawUnitLine{CE} = \mbox{ y } \parallel \drawUnitLine{DF}$ \byref{prop:I.XXXIII}
...
en la misma base (\drawUnitLine{CD}) y entre las mismas paralelas son iguales.
}

\begin{center}
$\left.
	\begin{aligned}
		\mbox{Dibuja } \drawUnitLine{AC} &\parallel \drawUnitLine{BD}\\
		\drawUnitLine{FD} &\parallel \drawUnitLine{EC}\\
	\end{aligned}
\right\}\mbox{\byref{prop:I.XXXI}}$

Prolonga \drawUnitLine{HI}.

\drawPolygon[bottom][polygonABDC]{ABC, BCG, CDG}
y
\drawPolygon[bottom][polygonEFDC]{DGE, CDG, EFD}
son paralelogramos en la misma base y entre las mismas paralelas, y por lo tanto iguales. \byref{prop:I.XXXV}

$\therefore \left\{
	\begin{aligned}
		\polygonABDC &= \mbox{ dos veces } \polygonBCD\\
		\polygonEFDC &= \mbox{ dos veces } \polygonCDE\\
	\end{aligned}
...
\drawCurrentPictureInMargin
\problem{T}{riángulos}{(\drawPolygon[bottom][polygonBCD]{BCD} y \drawPolygon[bottom][polygonEGH]{EGH}) en bases iguales y entre las mismas paralelas son iguales.}
...
\drawCurrentPictureInMargin
\problem{T}{riángulos}{ iguales (\drawPolygon[bottom][polygonADC]{AEC, ECD} y~\drawPolygon[bottom][polygonBDC]{BED, ECD}) sobre la misma base (\drawUnitLine{CD}) y en el mismo lado, están entre las mismas paralelas.}

\begin{center}
Si \drawUnitLine{AB}, que une los vértices de los triángulos, no sea $\parallel \drawUnitLine{CD}$, dibuja $\drawUnitLine{AF} \parallel \drawUnitLine{CD}$ \byref{prop:I.XXXI}, encontrando \drawUnitLine{CG}.

Dibuja \drawUnitLine{DF}.

$\because \drawUnitLine{AF} \parallel \drawUnitLine{CD}$ \byref{\constref}\\
$\polygonADC = \drawPolygon[bottom][polygonFDC]{BED, ECD, FBD}$ \byref{prop:I.XXXVII};\\
pero $\polygonADC = \polygonBDC$ \byref{\hypref};

$\therefore \polygonBDC = \polygonFDC$, una parte igual al todo, lo cual es absurdo.
...
\begin{center}
Si \drawSizedLine{AB} que une los vértices de los triángulos no sea $\parallel \drawSizedLine{CD,DE,EF}$,\\
dibuja \drawSizedLine{AG} $\parallel \drawSizedLine{CD,DE,EF}$ \byref{prop:I.XXXI}, encontrando \drawSizedLine{EH}.\\
Dibuja \drawSizedLine{FG}.

$\because \drawSizedLine{AG} \parallel \drawSizedLine{CD,DE,EF}$ \byref{\constref}\\
$\polygonACD = \drawFromCurrentPicture[bottom][polygonGEF]{
    startAutoLabeling;
...
$\therefore \polygonBEF = \polygonGEF$, una parte igual al todo, lo cual es absurdo.

$\therefore \drawSizedLine{AG} \nparallel \drawSizedLine{CD,DE,EF}$: y de la misma manera se puede demostrar que ninguna otra línea excepto \drawSizedLine{AB} es $\parallel \drawSizedLine{CD,DE,EF}$: $\therefore \drawSizedLine{AB} \parallel \drawSizedLine{CD,DE,EF}$.
...
\drawCurrentPictureInMargin
\problem{S}{i}{ un paralelogramo \drawPolygon[bottom][polygonABED]{ABFG,GFE,AGD,DEG} y un triángulo \drawPolygon[bottom][polygonCED]{GFE,DEG,CFE} están sobre la misma base \drawUnitLine{DE} y entre las mismas paralelas \drawUnitLine{AC} y \drawUnitLine{DE}, el paralelogramo es el doble del triángulo.}

\begin{center}
Dibuja la diagonal \drawUnitLine{AE}.

Entonces $\drawPolygon[bottom][polygonAED]{AGD,DEG} = \polygonCED$ \byref{prop:I.XXXVII}\\
$\polygonABED = \mbox{ dos veces } \polygonAED$ \byref{prop:I.XXXIV}

$\therefore \polygonABED = \mbox{ dos veces } \polygonCED$.
...
\drawCurrentPictureInMargin
\problem{L}{os}{complementos (\drawPolygon[bottom][polygonHBGE]{HBGE} y \drawPolygon[bottom][polygonFCIE]{FCIE}) de los paralelogramos que están alrededor de la diagonal de cualquier paralelogramo, son iguales entre sí.}
...
\drawCurrentPictureInMargin
\problem{A}{}{ una línea recta dada (\drawUnitLine{EG}) para aplicar un paralelogramo igual a un triángulo dado (\drawPolygon[middle][polygonJKL]{JKL}), y teniendo un ángulo igual a un ángulo rectilíneo dado (\drawAngle{N}).}
...
\problem{P}{ara}{ construir un paralelogramo igual a una figura rectilínea dada (\drawPolygon[middle][polygonABCDE]{ABC,ACD,ADE}) y que tenga un ángulo igual a un ángulo rectilíneo dado (\drawAngle{O}).}

\begin{center}
Dibuja \drawUnitLine{AD} y \drawUnitLine{AC}, dividiendo la figura rectilínea en triángulos.

Construye el paralelogramo $\drawPolygon{FGIH} = \drawPolygon{ADE}$ \byref{prop:I.XLII}\\
teniendo $\drawAngle{I} = \drawAngle{O}$;\\
para \drawUnitLine{GI} aplica el paralelogramo $\drawPolygon{GJKI} = \drawPolygon{ACD}$ \byref{prop:I.XLIV}\\
teniendo $\drawAngle{K} = \drawAngle{O}$;\\
y para \drawUnitLine{JK} aplica el paralelogramo $\drawPolygon{JLMK} = \drawPolygon{ABC}$ \byref{prop:I.XLIV}\\
...
stopTempAngleScale;
                     ...
                     \\ y $\therefore \polygonABDC$ es un cuadrado. \byref{def:I.XXX}
...
\begin{center}
Sobre \drawUnitLine{BC}, \drawUnitLine{CA}, \drawUnitLine{AB} describe cuadrados, \byref{prop:I.XLVI}

Dibuja $\drawUnitLine{AK} \parallel \drawUnitLine{CI}$ \byref{prop:I.XXXI}\\
también dibuja \drawUnitLine{BF} y \drawUnitLine{AI}.\\
$\drawAngle{BCI} = \drawAngle{FCA}$.

A cada uno añade \drawAngle{ACB} $\therefore \drawAngle{BCI,ACB} = \drawAngle{FCA,ACB}$,\\
...
De nuevo, $\because \drawUnitLine{AB} \parallel \drawUnitLine{CF}$\\
$\drawPolygon[middle][polygonACFG]{LAGF,CLF} = \mbox{ dos veces } \polygonBLC$,\\
y $\drawPolygon[middle][polygonJMCK]{JMIK,MCI} = \mbox{ dos veces } \polygonAFC$;

$\therefore \polygonACFG = \polygonJMCK$.

De la misma manera se puede mostrar que $\drawPolygon[middle][polygonABED]{ABED} = \drawPolygon[middle][polygonBJKH]{BJKH}$;
